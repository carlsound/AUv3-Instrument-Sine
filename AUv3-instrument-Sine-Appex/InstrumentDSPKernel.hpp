...
public:
	...
	struct NoteState {
		...
		void noteOn(int noteNumber, int velocity)
		{
			if (velocity == 0) {
                ...
                ampL = amp; //amp * panValue(-pan);
                ampR = amp; //amp * panValue(pan);
				oscPhase = 0.;
				stage = stageAttack;
	...
	void setBuffers(AudioBufferList* outBufferList) {
        ...
        maximillianSettings->setup(sampleRate, outBufferListPtr->mBuffers->mNumberChannels, outBufferListPtr->mBuffers->mDataByteSize);
	}
